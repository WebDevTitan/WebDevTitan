namespace Project.Bookie
{
    ...
    class BetplayCDPCtrl : IBookieController
    {
...
#pragma warning restore 649
        ...
        public bool login()
        {
            ...
            while(retryCnt++ < 3)
            {
                try
                {
                    ...
                    CDPController.Instance.NavigateInvoke($"https://{domain}/apuestas#home");
                    ...
                    {
                        Thread.Sleep(500);
                        string result = CDPController.Instance.ExecuteScript("document.getElementById('dropdownLogin').outerHTML;", true);
                    ...
                    double innerWidth = Utils.ParseToDouble(CDPController.Instance.ExecuteScript("window.innerWidth", true));
                    if (innerWidth <= 800)
                    {
                        isClicked = CDPController.Instance.FindAndClickElement(documentNodeId, "button[id='dropdownLogin']").Result;
                        Thread.Sleep(1000);
                    }

                    isClicked = CDPController.Instance.FindAndClickElement(documentNodeId, "input[id='userName']", 3).Result;
                    Thread.Sleep(1000);

                    CDPMouseController.Instance.InputText(Setting.Instance.username);

                    Thread.Sleep(1000);
                    CDPMouseController.Instance.PressKeyboard("");
                    Thread.Sleep(1500);

                    isClicked = CDPController.Instance.FindAndClickElement(documentNodeId, "input[id='password']", 3).Result;
                    Thread.Sleep(1000);

                    //isClicked = CDPController.Instance.CLickOnPoint((int)iconRect.X, (int)iconRect.Y , Json.ClickType.TripleClick).Result;
                    //Thread.Sleep(1000);
                    CDPMouseController.Instance.InputText(Setting.Instance.password);
                    Thread.Sleep(1000);

                    if (innerWidth <= 800)
                        isClicked = CDPController.Instance.FindAndClickElement(documentNodeId, "button[id='btnLoginPrimaryModal']", 2).Result;
                    else
                        isClicked = CDPController.Instance.FindAndClickElement(documentNodeId, "button[id='btnLoginPrimary']", 2).Result;
                    ...
                    if (!string.IsNullOrEmpty(CDPController.Instance.auth_token))
                    {
                        LogMng.Instance.onWriteStatus("Capturing Hash value success");
                        return true;
                    }
        ...
        public PROCESS_RESULT PlaceBet(ref BetburgerInfo info)
        {
            ...
            try
            {
                ...
                List<OpenBet_Unibet> markets = new List<OpenBet_Unibet>();
                try
                {
                    markets = new List<OpenBet_Unibet> { Utils.ConvertBetburgerPick2OpenBet_Unibet(info) };
                    
                }
                catch(Exception e)
                {
                    LogMng.Instance.onWriteStatus(string.Format("** Parsing Open_Unibet {0}", e.StackTrace));
                }
        ...
        public double getBalance()
        {
            ...
            while (--retryCount >= 0)
            {
                try
                {
                    string result = CDPController.Instance.ExecuteScript("localStorage.balance", true);
        ...
        private bool AddBetSlip(List<OpenBet_Unibet> markets)
        {
            ...
            {
                LogMng.Instance.onWriteStatus("AddBetSlip Failed " + e.StackTrace);
                return false;
            }
        ...
        private bool OddCheck(List<OpenBet_Unibet> markets)
        {
            try
            {
                ...
                m_client.DefaultRequestHeaders.TryAddWithoutValidation("Authorization", string.Format("Bearer {0}", CDPController.Instance.auth_token));
            ...
            {
                LogMng.Instance.onWriteStatus("OddCheck Failed " + e.StackTrace);
                return false;
            }
        ...
        private bool FinalBet(List<OpenBet_Unibet> markets, string stakeStr)
        {
            ...
            {
                LogMng.Instance.onWriteStatus("FinalBet Failed " + e.InnerException.Message);
            }
            return false;
        }
