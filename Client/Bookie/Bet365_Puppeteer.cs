namespace Project.Bookie
{
    ...
    public class BET365_PUPPETEERCtrl : IBookieController
    {
...
#pragma warning restore 649
        ...
        private void RunBrowser()
        {
            ...
            dynamic jsonBetsapiResp = null;
            HttpClient httpclient = new HttpClient();
            
            try
            {
                HttpResponseMessage responseMessageLogin = httpclient.GetAsync("http://127.0.0.1:9222/json/version").Result;
                responseMessageLogin.EnsureSuccessStatusCode();
                string responseMessage = responseMessageLogin.Content.ReadAsStringAsync().Result;

                jsonBetsapiResp = JsonConvert.DeserializeObject<dynamic>(responseMessage);
            }
            catch
            {
                if (FindWindow("Chrome_WidgetWin_1", null) != IntPtr.Zero)
                {

                    System.Diagnostics.Process.Start("C:\\Program Files\\Mozilla Firefox\\firefox.exe", "--remote-debugging-port=9222");
                    //System.Diagnostics.Process.Start("C:\\Program Files\\Mozilla Firefox\\firefox.exe", "-start-debugger-server 9222");

                    Thread.Sleep(3000);
                }
                //browser = Puppeteer.LaunchAsync(new LaunchOptions
                //{
                //    DefaultViewport = new ViewPortOptions { Width = 1000, Height=600 },
                //    Headless = false,
            ...
            //{
            //    BrowserWSEndpoint = jsonBetsapiResp.webSocketDebuggerUrl.ToString()
            //};

            browser = Puppeteer.ConnectAsync(new ConnectOptions() { BrowserWSEndpoint = jsonBetsapiResp.webSocketDebuggerUrl }).Result;
            //browser = Puppeteer.ConnectAsync(options).Result;
        ...
        private async void Router(object sender, RequestEventArgs route)
        {
            ...
            try
            {
                if (method == HttpMethod.Post)
                {
                    if (url.Contains("/members/lp/default.aspx"))
                    {
                        ...
                        postdata = Utils.ReplaceStr(postdata.ToString(), ecUsername, "&txtUsername=", "&");
                        postdata = Utils.ReplaceStr(postdata.ToString(), ecPassword, "&txtPassword=", "&");                        
                    }
            ...
            route.Request.ContinueAsync(new Payload() { Method = method, PostData = postdata==null?null:postdata.ToString(), Headers = headers, Url = url });
        }
        ...
        private async void Page_Response(object sender, ResponseCreatedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    wait_AddbetResult = await e.Response.TextAsync();
#if (TROUBLESHOT)
                ...
                {
                    wait_PlacebetResult = await e.Response.TextAsync();
#if (TROUBLESHOT)
        ...
        public string getProxyLocation()
        {
            try
            {
                ...
                try
                {
                    ...
                    string content = page.QuerySelectorAsync("xpath=//body").Result.ToString().Replace("Current IP Address:", "");
                    return content;
                }
                catch (Exception ex) { }
            }
        ...
        private bool PageIsVisible(string param)
        {
            ...
            try
            {
                ...
                bResult = page.WaitForSelectorAsync(param, options).Result != null;
            }
            catch { }
            return bResult;
        }
        private bool PageClick(string param, int timeout = 500, int nRetry = 3)
        {
            ...
            while (nRetry-- > 0)
            {
                try
                {
                    if (PageIsVisible(param))
                    {

                        BoundingBox rect = page.QuerySelectorAsync(param).Result.BoundingBoxAsync().Result;
        ...
        {
            string result = page.EvaluateFunctionAsync(function, param).Result.ToString();
            return result;  
        }
        private string RunScript(string param)
        {
            ...
            try
            {
                result = page.EvaluateExpressionHandleAsync(param).Result.ToString().ToLower();
            }
            catch { }
            return result;
        }
            ...
            try
            {
                ...
                while (nTotalRetry++ < 3)
                {
                    ...
                    {//return Locator.user.isLoggedIn;
                        ...
                        //PageClick("input.lms-StandardLogin_Username");
                        page.Keyboard.TypeAsync(Setting.Instance.username, new PuppeteerSharp.Input.TypeOptions() { Delay = 100 }).Wait();
                        Thread.Sleep(500);

                        PageClick("input.lms-StandardLogin_Password");
                        //PageClick($"input.lms-StandardLogin_Password");
                        page.Keyboard.TypeAsync(Setting.Instance.password, new PuppeteerSharp.Input.TypeOptions() { Delay = 100 }).Wait();
                        Thread.Sleep(500);
        ...
        public PROCESS_RESULT PlaceBetInBrowser(BetburgerInfo info)
        {
            ...
            try
            {
                ...
                while (nTotalRetry < 2)
                {
                    ...
                    try
                    {
                        BoundingBox rect = new BoundingBox(0, 0, 0, 0);
                        if (info.kind == PickKind.Type_6) //prematch
                            rect = page.QuerySelectorAsync("div.wcl-PageContainer").Result.BoundingBoxAsync().Result;
                        else
                            rect = page.QuerySelectorAsync("div.ipe-EventViewDetail").Result.BoundingBoxAsync().Result;
            ...
            while (true)
            {
                ...
                if (nTaskListcount < 1)
                {
                    ...
                    if (nActKind < 3)
                    {
                        try
                        {
                            int x = rnd.Next(1, page.Viewport.Width);
                            int y = rnd.Next(1, page.Viewport.Height);
                            MoveMouse(x, y);
                        }
                        catch { }
                    }
