...
{
#if (SEUBET)
    class SeubetCtrl : IBookieController
    {
        ...
        public void startListening()
        {
            try
            {
                ...
                // Customize WebSocket
                _webSocket.OnMessage += doLogin;
                _webSocket.OnMessage += placeBet;
        ...
        private void sendRequestPlaceBet(double price, long eventId)
        {
            ...
            LogMng.Instance.onWriteStatus("Bet Request Success");
        }
        private void placeBet(object sender, MessageEventArgs e)
        {
            try
            {
                JObject jsonData = JObject.Parse(e.Data);
                ...
                if (resultToken != null && resultToken.ToString() == "OK")
                {
            ...
            {
                LogMng.Instance.onWriteStatus($"Exception in placeBet: {ex.Message}");
            }
        }
        ...
        public bool login()
        {
            sendRequestLogin();
            Thread.Sleep(3000);
            if (m_userId == null) return false;
            else return true;
        }

        public void doLogin(object sender, MessageEventArgs e)
        {
            try
            {
                JObject jsonData = JObject.Parse(e.Data);
        ...
        {
            sendRequestPlaceBet(info.odds, info.eventid);
            Thread.Sleep(1000);

            return PROCESS_RESULT.PLACE_SUCCESS; // Example return value
        }
