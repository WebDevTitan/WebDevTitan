...
{
#if (STOIXIMAN_CDP || BETANO_CDP)
    class Stoiximan_CDP : IBookieController
    {
...
#if (CHRISTIAN)
            if(!domain.Contains("stoiximan.gr"))
            ...
            { 
                ...
                try
                {
                    ...
                    lock (lockerObj)
                    {
                        ...
                        Thread.Sleep(3000);
                        CDPController.Instance.NavigateInvoke($"https://{domain}/myaccount/login");
                        Thread.Sleep(15000);

                        CDPController.Instance.ClickCaptcha().Wait();

                        long documentId = CDPController.Instance.GetDocumentId().Result;
                        bool isFound = CDPController.Instance.FindAndClickElement(documentId, "input[id='username']").Result;
                        Thread.Sleep(1500);
                        ...
                        isFound = CDPController.Instance.FindAndClickElement(documentId, "button[data-qa='submit']").Result;
                        Thread.Sleep(2000);
        ...
        {            
            ...
            try
            {
                lock (lockerObj)
                {
                    ...
                    string ReqJson = $"{{\"selectionIds\":[\"{selectionId}\"],\"betslip\":{{\"hash\":\"\",\"slipData\":\"\",\"legs\":[],\"bets\":[],\"betslipTabId\":1,\"betslipTrackId\":\"\"}}}}";
        ...
        public double getBalance()
        {
            ...
            {                                
                lock (lockerObj)
                {
                    ...
                    dynamic jsonContent = JsonConvert.DeserializeObject<dynamic>(CDPController.Instance.balanceRespBody);
                    if (domain.Contains("betano"))
                    {                        
