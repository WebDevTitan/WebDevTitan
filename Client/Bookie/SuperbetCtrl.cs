...
{
#if (SUPERBET)
    class SuperbetCtrl : IBookieController
    {
        ...
        public SuperbetCtrl()
        {
            ...
            if (Setting.Instance.browserType == 0)
            {
                if (CDPController.Instance._browserObj == null)
                    CDPController.Instance.InitializeBrowser("https://superbet.com/");
            }
            else
            {
                if (DolphinController.Instance.browser == null)
                    DolphinController.Instance.InitBrowser("https://superbet.com/");
            }
        ...
        public bool Pulse()
        {
            ...
            else
            {
                if (DolphinController.Instance.browser != null)
                {
                    DolphinController.Instance.ExecuteScript("document.getElementsByClassName('sb-btn sb-btn--block sb-btn--large sb-btn--success')[0].click()");
                    Thread.Sleep(2000);
                    string cur_url = DolphinController.Instance.ExecuteScript("location.href");
                    if (cur_url.Contains("pt-br/401"))
                        DolphinController.Instance.NavigateInvoke("https://superbet.com");
                    else
                        DolphinController.Instance.NavigateInvoke("https://superbet.com/pt-br/profile/withdrawal");
                }
                return true;
            }
...
#pragma warning restore 649
            ...
            while (nTotalRetry++ < 3)
            {
                ...
                try
                {
                    lock (lockerObj)
                    {
                        ...
                        isFound = CDPController.Instance.FindAndClickElement(documentId, "button[class='sds-button sds-focus e2e-login sds-button--md sds-button--tertiary-color']").Result;
                        if (!isFound)
                        {
                        ...
                        CDPController.Instance.ExecuteScript(doEventJScript);

                        string input_name_script = $"var el = document.getElementsByClassName('input__field')[0];el.value = '{Setting.Instance.username}';doEvent(el, 'input');";
                        CDPController.Instance.ExecuteScript(input_name_script);

                        string input_password_script = $"var el = document.getElementsByClassName('input__field')[1];el.value ='{Setting.Instance.password}';doEvent(el, 'input');";
                        CDPController.Instance.ExecuteScript(input_password_script);

                        CDPController.Instance.ExecuteScript("document.getElementsByClassName('sb-btn e2e-login-submit-btn sb-btn--block sb-btn--large sb-btn--primary')[0].click()");
            ...
            while (nTotalRetry++ < 3)
            {
                ...
                try
                {
                    lock (lockerObj)
                    {
                        ...
                        //Click Session Close button
                        DolphinController.Instance.FindAndClick("div[class='bss-ReceiptContent_Done ']");
                        DolphinController.Instance.FindAndClick("button[class='capitalize btn btn--primary btn--block']");

                        //Click Session Continue Button
                        DolphinController.Instance.ExecuteScript("document.getElementsByClassName('sb-btn sb-btn--block sb-btn--large sb-btn--success')[0].click()");

                        bool isFound = DolphinController.Instance.FindAndClick("button[class='sds-button sds-focus e2e-login sds-button--md sds-button--primary-color']");
                        if (!isFound)
                            isFound = DolphinController.Instance.FindAndClick("div[class='login-banner mb-3 e2e-login-banner']");
                        ...
                        Thread.Sleep(1500);
                        DolphinController.Instance.ExecuteScript("document.getElementById('login-modal-submit').click()");

                        Thread.Sleep(10000);
                        int retryCnt = 0;
                        while (!DolphinController.Instance.isLogged)
                        {
                            retryCnt++;
                            Thread.Sleep(500);
                            if (retryCnt > 30)
                        ...
                        bLogin = DolphinController.Instance.isLogged;
                        if (!bLogin)
                        {
                            bLogin = getBalance() > 0 ? true : false;
                            if (!bLogin)
                                continue;
                        }
                        break;
                ...
                lock (lockerObj)
                {
                    ...
                    while(kk < 3)
                    {
                        ...
                        if (Setting.Instance.browserType == 0)
                        {
                            ...
                            jResp = JObject.Parse(CDPController.Instance.PlaceBetRespBody);
                        }
                        else
                        {
                            DolphinController.Instance.PlaceBetRespBody = string.Empty;
                            DolphinController.Instance.ExecuteScript(functionString);
                            ...
                            if (string.IsNullOrEmpty(DolphinController.Instance.PlaceBetRespBody))
                            {
                                kk++;
                                continue;
                            }

                            jResp = JObject.Parse(DolphinController.Instance.PlaceBetRespBody);
                        }
                        ...
                        if ((bool)jResp["error"])
                        {
                            if (jResp["errorCode"].ToString() == "liabilityLimitsExceeded")
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        //TelegramCtrl.Instance.sendMessage($"Possible limitation on the {Setting.Instance.username} account on the Superbet website");
        ...
        public double getBalance()
        {
            ...
            try
            {
                lock (lockerObj)
                {
                    string fetch_script = File.ReadAllText("balance.txt");

                    if(Setting.Instance.browserType == 0)
                    {
                        CDPController.Instance.balanceRespBody = string.Empty;
                        CDPController.Instance.ExecuteScript(fetch_script);
                        ...
                        balance = (double)jResp["data"]["cash"]["total"];
                    }
                    else
                    {
                        DolphinController.Instance.balanceRespBody = string.Empty;
                        DolphinController.Instance.ExecuteScript(fetch_script);

                        int retryCnt = 0;
                        while (string.IsNullOrEmpty(DolphinController.Instance.balanceRespBody))
                        {
                            retryCnt++;
                            if (retryCnt > 20)
                                break;
                        ...
                        JObject jResp = JObject.Parse(DolphinController.Instance.balanceRespBody);
                        balance = (double)jResp["data"]["cash"]["total"];
                    }
        ...
        public SuperbetTicket FindEvent(string eventId, string outcomeId , string sbValue)
        {
            SuperbetTicket ticket = new SuperbetTicket();
            try
            {
                string fetchScript = File.ReadAllText("GetFetch.txt");
                fetchScript = fetchScript.Replace("[url]", $"https://production-superbet-offer-basic.freetls.fastly.net/sb-basic/api/v2/en-BR/events/{eventId}?matchIds={eventId}");
                JObject jResp = new JObject();
                ...
                if (Setting.Instance.browserType == 0)
                {
                    ...
                    jResp = JObject.Parse(CDPController.Instance.eventRespBody);
                }
                else
                {
                    DolphinController.Instance.eventRespBody = string.Empty;
                    int retryCnt = 0;
                    DolphinController.Instance.ExecuteScript(fetchScript);
                    ...
                    jResp = JObject.Parse(DolphinController.Instance.eventRespBody);
                }
        ...
        public void GetTokens()
        {
            try
            {
                ...
                {
                    device_id = DolphinController.Instance.ExecuteScript("localStorage.device_id");
                    anonymous_id = DolphinController.Instance.ExecuteScript("localStorage.ldAnonymousUserKey");
                }
