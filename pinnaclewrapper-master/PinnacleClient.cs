...
namespace PinnacleWrapper
{
    public class PinnacleClient
    {
        ...
        public PinnacleClient(string clientId, string password, string currencyCode, OddsFormat oddsFormat ,  Proxy proxy)
        {
            _clientId = clientId;
            _password = password;
            ...
            if (proxy != null)
            {
                var proxyURI = new Uri(string.Format("{0}{1}:{2}", "http://", proxy.proxyIP, proxy.port));
                WebProxy proxyItem = null;
                var useAuth = false;
                ...
                handler.Proxy = proxyItem;
                handler.UseDefaultCredentials = !useAuth;
                handler.PreAuthenticate = useAuth;
            }
        ...
        public Task<GetSpFixturesResponse> GetSpecialFixtures(GetFixturesRequest request)
        {
            var sb = new StringBuilder();

            sb.AppendFormat("v1/fixtures/special?sportId={0}", request.SportId);
            ...
            return GetJsonAsync<GetSpFixturesResponse>(sb.ToString());
        }
        ...
        public Task<GetSpOddsResponse> GetSpecialOdds(GetOddsRequest request)
        {
            var sb = new StringBuilder();

            sb.AppendFormat("v1/odds/special?sportId={0}", request.SportId);
            ...
            return GetJsonAsync<GetSpOddsResponse>(sb.ToString());
        }
    }
}
