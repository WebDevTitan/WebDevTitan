...
namespace Bet365LiveAgent.Data.Soccer
{
    ...
    [Serializable]
    public class GoalLineMarketData
    {        
        public List<GoalLineMarket> MarketData { get; set; } = new List<GoalLineMarket>();
        ...
        public void Update(JObject jObjData)
        {
            ...
            else if ("MA".Equals(type))
            {
                foreach (var jPAToken in jObjData["PAData"].ToObject<JObject>())
                {
                    ...
                    {
                        GoalLineMarket goalLineMarket = MarketData.Find(m => !string.IsNullOrWhiteSpace(m.UnderHdp) && decimal.Parse(m.UnderHdp) == jPAData["HA"].ToObject<decimal>());
                    ...
                    else if ("3".Equals(jObjData["ID"].ToString()))
                    {
                        GoalLineMarket goalLineMarket = MarketData.Find(m => !string.IsNullOrWhiteSpace(m.OverHdp) && decimal.Parse(m.OverHdp) == jPAData["HA"].ToObject<decimal>());
                        if (goalLineMarket == null)
                        {
                            goalLineMarket = new GoalLineMarket();
                            MarketData.Add(goalLineMarket);
